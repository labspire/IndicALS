...
CHUNK_DURATION = 2.0
CHUNK_SAMPLES = int(CHUNK_DURATION * SR)  # 16000 samples
HOP_SAMPLES = int(1.0 * SR)               # 1 second step (50% overlap)
...
def read_segments(txt_path):
    segments = []
    if not os.path.exists(txt_path): return segments
...
class ConvLSTMModel(nn.Module):
    def __init__(self):
        ...
        # Classification Dense Output
        self.fc = nn.Linear(150, 2)

    def forward(self, x):
        batch_size, time_steps, sequence_length, channels = x.size()
        
        # Reshape to process frames independently via Conv1D
        x = x.view(-1, sequence_length, channels)        # [B * 200, 160, 1]
        x = x.permute(0, 2, 1)                           # [B * 200, 1, 160]

        x = self.conv1(x)                                # [B * 200, 256, 41]
        x = self.relu(x)
        x = self.batch_norm1(x)
        x = self.maxpool1(x)                             # [B * 200, 256, 1]
        
        x = x.squeeze(-1)                                # [B * 200, 256]

        # Reconstruct sequence dimension
        x = x.view(batch_size, time_steps, 256)          # [B, 200, 256]
        x = x.permute(0, 2, 1)                           # [B, 256, 200]
        
        x = self.conv2(x)                                # [B, 30, Out_Steps]
        x = self.relu(x)
        x = self.maxpool2(x)                             # [B, 30, Pooled_Steps]
...
if __name__ == "__main__":
    ...
    excluded_phonemes = {"SSS", "SHS", "FFF"}

    for root, label in [(ALS_ROOT, 1), (HC_ROOT, 0)]:
        sub_dirs = [d for d in glob.glob(os.path.join(root, "*")) if os.path.isdir(d)]
        for s_dir in tqdm(sub_dirs, desc=f"Processing {'ALS' if label==1 else 'HC'} 2s Word Chunks"):
            suv_path = os.path.join(s_dir, "SUV")
            if not os.path.isdir(suv_path):
                continue
                
            sid = clean_subject_id(os.path.basename(s_dir))
            
            for t_path in glob.glob(os.path.join(suv_path, "*.txt")):
                filename = os.path.basename(t_path)
                
                if any(ex_phon in filename for ex_phon in excluded_phonemes):
                    continue
                ...
                try:
                    ...
                    for start, end in segments:
                        ...
                        # Case A: Segment < 2s -> Zero padding
                        ...
                        # Case B: Segment >= 2s -> Sliding window (50% overlap)
                        else:
    ...
    print("\n" + "="*40)
    print("   CHUNKING DATASET SUMMARY")
    print("   (EXCLUDED: SSS, SHS, FFF)")
    print("="*40)
    print(f"Total 2-Second Chunks: {len(master_df)}")
    print(f"Total Unique Subjects:  {master_df['subject'].nunique()}")
    print("-" * 40)
    print(f"ALS Chunks (Label 1):   {len(master_df[master_df['label']==1])}")
    print(f"HC Chunks (Label 0):   {len(master_df[master_df['label']==0])}")
    print("="*40 + "\n")

    if len(master_df) == 0:
        raise ValueError("No valid chunks extracted! Check paths, exclusion rules, and text boundary files.")
    ...
    for f in range(1, 6):
        ...
        print(f"--- FOLD {f} STATISTICS ---")
        print(f"Training:   {len(df_train)} chunks from {len(tr_s)} subjects")
        print(f"Validation: {len(df_val)} chunks from {len(vl_s)} subjects")
        print(f"Testing:    {len(df_test)} chunks from {len(te_s)} subjects")
        print("-" * 30)

        train_ds = suvRawDataset(df_train)
        val_ds = suvRawDataset(df_val)
        test_ds = suvRawDataset(df_test)
    ...
    with open(f"{RESULTS_DIR}/final_summary.txt", "w") as sf:
        sf.write("Cross-Validation Complete Operational Summary\n")
        sf.write("="*60 + "\n")
    ...
    print(f"\nTraining Complete. Global Cross-Validation Summary Saved.")
    print(f"Final File-Level Avg Accuracy: {np.mean(file_accuracies):.4f} (SD: {np.std(file_accuracies):.4f})")
    print("All tracking files successfully generated inside directory:", RESULTS_DIR)