...
if __name__ == "__main__":
    ...
    for root, label in [(ALS_ROOT, 1), (HC_ROOT, 0)]:
        ...
        for s_dir in tqdm(sub_dirs, desc=f"Processing {'ALS' if label==1 else 'HC'}"):
            ddk_path = os.path.join(s_dir, "DDK")
            if not os.path.isdir(ddk_path):
                continue
                
            sid = clean_subject_id(os.path.basename(s_dir))
            
            for t_path in glob.glob(os.path.join(ddk_path, "*.txt")):
    ...
    if len(master_df) == 0:
        raise ValueError("No valid chunks extracted! Check dataset directory paths.")
    ...
    for f in range(1, 6):
        ...
        train_ds = DDKRawDataset(df_train)
        val_ds = DDKRawDataset(df_val)
        test_ds = DDKRawDataset(df_test)
        ...
        y_te_true, y_te_pred = [], []
        with torch.no_grad():
            for x, y in tqdm(te_loader, desc=f"Testing Fold {f}"):
                out = model(x.to(DEVICE))
        ...
        with open(f"{RESULTS_DIR}/fold_{f}_results.txt", "w") as rf:
            rf.write(f"Fold {f} Evaluation Summary\n")
            rf.write("="*35 + "\n")
            rf.write("[CHUNK-LEVEL]\n")
            rf.write(f"Accuracy: {t_acc:.4f}\n")
            rf.write(f"F1 Score: {t_f1:.4f}\n")
            rf.write(f"Confusion Matrix:\n{t_cm}\n\n")
            rf.write("[FILE-LEVEL MAJORITY VOTING]\n")
    ...
    with open(f"{RESULTS_DIR}/final_summary.txt", "w") as sf:
        sf.write("Cross-Validation Summary\n")
        sf.write("="*60 + "\n")
        sf.write(">> CHUNK-LEVEL STATS:\n")
        ...
        sf.write(">> FILE-LEVEL STATS (Majority Voting):\n")
    ...
    print(f"\nExecution complete. Summary saved to {RESULTS_DIR}/final_summary.txt")